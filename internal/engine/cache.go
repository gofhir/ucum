...
// load returns the cached expr.Term for a code, promoting it out of the old
// generation if that is where it was found.
func (c *termCache) load(code string) (*expr.Term, bool) {
...
// store caches a parsed expr.Term, rotating the generations if the young one is full.
func (c *termCache) store(code string, t *expr.Term) {
