...
func TestCaseInsensitiveVocabularyIsComplete(t *testing.T) {
	...
	for _, du := range ci.model.DefinedUnits {
		if du.CodeCI == "" {
	...
	for _, bu := range ci.model.BaseUnits {
	...
	for _, p := range ci.model.Prefixes {
...
func TestCaseInsensitiveServiceIsFullyFeatured(t *testing.T) {
	...
	f, err := ci.ConversionFactor("L", "ML")
	if err != nil {
		t.Fatal(err)
	}
	if f.RatString() != "1000" {
	...
	if ci.Definitions().Version == "" {
		t.Error("a case-insensitive service does not report its definitions")
	}
