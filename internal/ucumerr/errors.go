// Package ucumerr holds the error types and sentinels the ucum package
// publishes, in a leaf package so that every layer can construct them without
// importing the root and creating a cycle.
//
// The root re-exports all of it: the types as aliases, so errors.As sees one
// type, and the sentinels as copies of the same interface value, so errors.Is
// matches either name.
package ucumerr
...
// PositionError carries the position at which a code failed to lex or parse, so
// that ValidationError can report it as a number instead of leaving it buried in
// prose.
...
// At returns an error carrying the position it happened at.
func At(offset int, format string, args ...any) *PositionError {
	return &PositionError{offset: offset, msg: fmt.Sprintf(format, args...)}
}

// AtCause returns an error carrying both a position and the sentinel behind it.
func AtCause(offset int, cause error, format string, args ...any) *PositionError {
	return &PositionError{offset: offset, msg: fmt.Sprintf(format, args...), err: cause}
}
...
func WithPosition(err error, offset int) error {
	...
	return &PositionError{offset: offset, msg: err.Error(), err: err}
}

// Offset returns the position an error carries, or -1 if it carries none.
...
// Validation wraps an internal error as a ValidationError for the code it
...
// Conversion wraps an internal error as a ConversionError between two
// codes. An error that is already a ConversionError is returned unchanged.
func Conversion(from, to string, err error) error {
	if err == nil {
		return nil
	}
