// Portions of this file are ported from Lexer.java in FHIR/Ucum-java,
...
func (l *lexer) readAnnotation() error {
	...
	for l.index < len(l.source) {
		...
		if ch > 0x7E || ch < 0x20 {
			return ucumerr.At(l.index, "invalid character in annotation")
		}
		l.index++
	}

	return ucumerr.At(start, "unterminated annotation")
}
...
func (l *lexer) readSignedNumber() error {
	...
	if l.index >= len(l.source) || !isDigit(l.source[l.index]) {
		return ucumerr.At(start, "sign must be followed by a digit")
	}
...
func (l *lexer) readGeneralToken() error {
	...
	if inBracket {
		return ucumerr.At(start, "unterminated bracket")
	}

	if l.index == start {
		return ucumerr.At(l.index, "unexpected character %q", l.source[l.index])
	}
...
func (l *lexer) getTokenAsInt() (int, error) {
	...
	if err != nil {
		return 0, ucumerr.At(l.start, "token %q is not a valid integer", l.token)
	}
	return v, nil
}
