...
func TestErrorPathsNameTheOffendingCode(t *testing.T) {
	...
	cases := []struct {
		...
		{"ConvertRat bad source", func() error { _, err := exact.ConvertRat(one, badCode, "m"); return err }},
		{"ConvertRat bad target", func() error { _, err := exact.ConvertRat(one, "m", badCode); return err }},
		{"ConversionFactor bad source", func() error { _, err := exact.ConversionFactor(badCode, "m"); return err }},
		{"ConversionFactor bad target", func() error { _, err := exact.ConversionFactor("m", badCode); return err }},
		{"CanonicalRat", func() error { _, err := exact.CanonicalRat(one, badCode); return err }},
...
func TestErrorPathsOnValidButUncanonicalizableCodes(t *testing.T) {
	...
	cases := map[string]func() error{
		...
		"ConvertRat source":  func() error { _, err := exact.ConvertRat(one, "m/0", "m"); return err },
		"ConvertRat target":  func() error { _, err := exact.ConvertRat(one, "m", "m/0"); return err },
		"ConversionFactor":   func() error { _, err := exact.ConversionFactor("m/0", "m"); return err },
		"CanonicalRat":       func() error { _, err := exact.CanonicalRat(one, "m/0"); return err },
...
<root xmlns="http://unitsofmeasure.org/ucum-essence" version="1.0" revision="test"
   ...
   <unit Code="Xx" CODE="XX" isMetric="yes" isSpecial="yes" class="test">
	...
	_, err := NewFromReader(strings.NewReader(defs))
	if err == nil {
