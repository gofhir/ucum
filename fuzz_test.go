...
func FuzzValidate(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, code string) {
		...
		if err != nil {
			// Every rejection is a *ValidationError naming the code. Nothing
			// else may come out of Validate.
			var ve *ValidationError
			if !errors.As(err, &ve) {
				t.Fatalf("Validate(%q) returned %T, want *ValidationError: %v", code, err, err)
			}
		...
		if err != nil {
			var ve *ValidationError
			if !errors.As(err, &ve) {
				t.Fatalf("Canonical(1, %q) returned %T, want *ValidationError: %v", code, err, err)
			}
			return
		}
		if pair.Code == "" {
		...
		// is no identity to preserve on a degenerate scale, so ErrDivisionByZero
...
func FuzzComposeRoundTrip(f *testing.F) {
	...
	impl, ok := svc.(*service)
	if !ok {
		f.Fatalf("New returned %T", svc)
	}

	f.Fuzz(func(t *testing.T, code string) {
		t1, err := impl.parseCached(code)
		if err != nil {
			return
		}
		rendered := composeTerm(t1)
		if rendered == "" {
			return
		}

		t2, err := impl.parseCached(rendered)
		if err != nil {
			t.Fatalf("composeTerm(parse(%q)) = %q, which does not parse: %v", code, rendered, err)
		}
		...
		c1, err1 := impl.canonicalizeTerm(t1, impl.specialContextOf(t1))
		c2, err2 := impl.canonicalizeTerm(t2, impl.specialContextOf(t2))
		if err1 != nil || err2 != nil {
			return
		}
		if u1, u2 := composeCanonicalUnits(c1), composeCanonicalUnits(c2); u1 != u2 {
